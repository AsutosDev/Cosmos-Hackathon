...
import { Mail, Lock, ArrowRight, ArrowLeft, Package, User, Phone, MapPin, FileText, CheckCircle } from 'lucide-react';
import Link from 'next/link';

export default function RenterAuth() {
    ...
    try {
      if (isLogin) {
        ...
        await setDoc(doc(db, 'users', cred.user.uid), {
          firstName, lastName, mobile, address, businessType, email,
          role: 'renter',
  ...
  return (
    <div className="min-h-[90vh] flex items-center justify-center p-6 bg-white">
      ...
      >
        {/* Left panel — yellow */}
        <div className="w-[340px] shrink-0 bg-primary p-10 flex flex-col justify-between">
          <div>
            <Link href="/auth" className="flex items-center gap-2 text-white/70 text-sm mb-10 hover:text-white transition-colors">
              <ArrowLeft size={14} /> Back
            </Link>
            <div className="w-12 h-12 bg-white/20 border border-white/30 flex items-center justify-center mb-6">
              <Package size={24} className="text-white" />
            </div>
            <h2 className="text-3xl font-black text-white tracking-tight mb-3">Owner<br />Portal</h2>
            <p className="text-white/70 text-sm leading-relaxed">
              List your items, manage rentals, and earn income — all from one dashboard.
            </p>
          </div>

          <div className="flex flex-col gap-3">
            {['Set your own rental price', 'Track all active rentals', 'Get paid securely', 'Build your reputation'].map(f => (
            ...
            >
              {!isLogin && (
                <>
                  ...
                  <div>
                    <label className="block text-[0.7rem] font-bold text-text-muted uppercase tracking-wider mb-1.5">Address</label>
                    <div className="relative">
                  ...
                  <div>
                    <label className="block text-[0.7rem] font-bold text-text-muted uppercase tracking-wider mb-1.5">What will you rent out?</label>
                    <div className="relative">
                      <FileText size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
                      <input required type="text" placeholder="e.g. Camera gear, tools, vehicles…" value={businessType} onChange={e => setBusinessType(e.target.value)}
                        className="w-full border border-border py-2.5 pl-9 pr-3 text-sm outline-none focus:border-black transition-colors" />
                    </div>
                  </div>
                </>
              ...
              <button type="submit" disabled={loading}
                className="w-full bg-primary text-white py-3 font-bold text-sm flex items-center justify-center gap-2 hover:bg-primary-hover transition-colors disabled:opacity-50 mt-2">
                {loading ? 'Please wait…' : isLogin ? 'Sign In as Owner' : 'Create Owner Account'}
